...
public class CEducacion {
    ...
    public ResponseEntity<?> create(@RequestBody dtoEducacion dtoeducacion){
        if(StringUtils.isBlank(dtoeducacion.getDequien())){
            return new ResponseEntity(new Mensaje("El nombre es obligatorio"), HttpStatus.BAD_REQUEST);
        }
        if(sEducacion.existsByDequien(dtoeducacion.getDequien())){
    ...
    public ResponseEntity<?> update(@PathVariable("id") int id, @RequestBody dtoEducacion dtoeducacion){
        ...
        if(sEducacion.existsByDequien(dtoeducacion.getDequien()) && sEducacion.getByDequien(dtoeducacion.getDequien()).get().getId() != id){
            return new ResponseEntity(new Mensaje("Ese nombre ya existe"), HttpStatus.BAD_REQUEST);
        }
        if(StringUtils.isBlank(dtoeducacion.getDequien())){
        ...
        educacion.setDequien(dtoeducacion.getDequien());
        educacion.setCurso(dtoeducacion.getCurso());
        educacion.setFinalizado(dtoeducacion.getFinalizado());
